...
export const DISCIPLINE = {
  ...
  searchFirst: '任何录入前先 contact_search 定位联系人防建重；匹配到疑似同一人时向用户确认，而不是静默新建',
  ...
  multiPerson: '素材涉及多人时逐个 contact_search 定位，没有的联系人先与用户确认再新建',
  ...
  verbatim: '保留用户原话语义，不演绎、不补充；推断不当事实写；禁忌与健康信息只按用户原话记录为 taboo（不推断结论），importance=3；婚礼、住院等重大事件 importance=3，其余默认 2',
  ...
  materialOrganize: {
    ...
    build(materialId, toolsUrl) {
      ...
      return [
        ...
        `7. 每条用 memory_batch_add 登记并带 sourceId="${materialId}"；`,
        `8. 登记完列出拆出的记忆清单（含"哪些是已有记忆已覆盖、本次未重复登记"的说明），并提示我回工作台确认。`,
      ].join('\n');
    },
  },
  // 礼物建议（读路径 C）
...
// ── 出口派生：四处提示词全部从这里生成 ──────────────────────────────────
const TOOLS = ['contact_search', 'contact_add', 'contact_update', 'memory_add', 'memory_batch_add', 'memory_reject', 'memory_update', 'memory_search', 'timeline_get', 'gift_plan_add', 'gift_plan_list', 'gift_plan_update', 'gift_plan_delete', 'material_save', 'material_list', 'material_get'];

/** 工具清单段（播报/自足指令共用）：名字 + 一句话约束 */
export function toolCatalog() {
  return '可用工具：contact_search（查找联系人，任何录入前必调）、contact_add、contact_update、memory_add（登记一条待确认记忆，一条只含一个事实）、memory_batch_add（一段素材拆多条）、memory_reject（驳回待确认记忆须给理由——提取查重后清除本批重复条目时用）、memory_update、memory_search（生成祝福/礼物建议前必调，只返回已确认记忆，检索为空要明说）、timeline_get（读取某人时间线）、gift_plan_add（把礼物方案落成计划卡，理由须引用记忆点）、gift_plan_list、gift_plan_update、gift_plan_delete、material_save（存档用户粘贴的原始素材）、material_list（列出素材，用户说"整理素材"时先调）、material_get（读素材全文后提取；响应里的 today 字段是当天日期，相对时间一律以它为锚推算）。';
}
...
export function announcementBody() {
  const t = DISCIPLINE;
  return [
    `对话即录入：用户在会话里说出关系事实（如「记一下，小李女儿十月办婚礼，他对花生过敏」）时，你必须把每个事实登记为一条待确认记忆。涉及本工作台的全部操作经 REST API 完成，工具入口：POST {TOOLS_URL}，body 为 {"name":"工具名","args":{...}}。${toolCatalog()}${t.confirmHumanOnly}。`,
    `素材智能整理流程：用户粘贴长文本/聊天记录（无论在会话里还是工作台「智能整理」框里）→ 先 material_save 存档（可带 contactId 和 occasion 场景标签）→ material_get 读全文 → ${t.sceneFirst}；每个 fact 用 memory_batch_add 拆条登记，每条带 sourceId=素材 ID（未显式给 occasion 时自动继承素材的）；${t.dedupe}；${t.multiPerson} → 回复里列出拆出的记忆，提示用户回工作台确认。${t.dualTime}。三层标注：${t.direction}；${t.lifespan}；${t.occasion}`,
...
export function presetBody() {
  ...
  return [
    ...
    `录入纪律：对话中出现关系事实（事件、喜好、禁忌、礼物、承诺、往来、基础信息）时，必须把每个事实登记为一条待确认记忆（${t.onePerFact}）。所有写入经关系记忆工作台的 REST API 完成：工具入口 POST http://127.0.0.1:8901/api/tools，body 为 {"name":"工具名","args":{...}}；可用工具 ${TOOLS.join(' / ')}。${t.pendingOnly}；礼物计划是低风险意图，经 gift_plan_add 直接落卡（不进确认队列）；禁止声称"已记住"而未实际调用工具。${t.searchFirst}；确需新建时如实告知。工作台服务未启动或端口不通时，明确告知用户本条未登记，不要假装成功。`,
    `素材智能整理：用户粘贴长文本或聊天记录（无论是发在会话里，还是已通过工作台「智能整理」框保存到素材区）时，先 material_save 存档（可带 contactId 与 occasion 场景标签），再 material_get 读全文；${t.sceneFirst}。${t.dedupe}。每条用 memory_batch_add 拆成一条待确认记忆（每条带 sourceId=素材 ID 用于溯源）；${t.multiPerson}。用户说"整理素材"时先 material_list 找待整理（raw）的素材。整理完在回复里列出拆出的记忆（含"哪些是已有记忆已覆盖、本次未重复登记"的说明），并提示用户回工作台确认。`,
    `提取规范：${t.verbatim}。${t.dualTime}。三层标注（V4）：① ${t.direction} ② ${t.lifespan} ③ ${t.occasion} 硬规则：${t.behaviorOnly}；${t.conflict}`,
    `相对时间锚点：${t.dateAnchor}`,
    `应用纪律：${t.recallFirst}。用户明确说"确认/没错/就这么记"时，告知记忆已在待确认队列、请回工作台点击确认（${t.confirmHumanOnly}）。`,
    `${t.recallAvoidRepeat}`,
