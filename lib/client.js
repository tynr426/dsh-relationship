...
window.__ModuleLoader__.load({
  ...
  factory: function () {
    ...
    function ensureView() {
      ...
      frame.src = VIEW_URL;
      frame.title = '关系记忆工作台';
      // app.js 的删除确认已全部走页面内 confirmDialog（Electron 沙箱 iframe 里原生
      // confirm 的返回值经常拿不回来，会导致删除静默失效）；allow-modals 保留作兜底。
      frame.setAttribute('sandbox', 'allow-same-origin allow-scripts allow-forms allow-popups allow-downloads allow-modals');
    ...
    // 点击外链（商品/京东）→ 宿主页面代开新窗——沙箱 iframe 的 target=_blank
    // 会被桌面壳静默吞掉（用户点了没反应），宿主页面开窗与聊天内链接同通路。
    function onFrameMessage(event) {
      var data = event.data;
      if (!data || typeof data !== 'object') return;
      if (data.source === 'dsh-relationship' && data.type === 'close-panel') setActive(false);
      if (data.source === 'dsh-relationship' && data.type === 'open-external' && typeof data.url === 'string') {
        var url;
        try { url = new URL(data.url, location.href); } catch (e) { url = null; }
        if (url !== null && (url.protocol === 'https:' || url.protocol === 'http:') && !url.username && !url.password) {
          var opened = null;
          try { opened = window.open(url.href, '_blank'); } catch (e) { opened = null; }
          if (opened !== null && opened !== undefined) { try { opened.opener = null; } catch (e) { /* ignore */ } }
          else {
            var frameEl = viewRoot !== undefined && viewRoot.isConnected ? viewRoot.querySelector('.rel-frame') : null;
            if (frameEl !== null && frameEl.contentWindow !== null) frameEl.contentWindow.postMessage({ source: 'dsh-relationship-host', type: 'open-external-blocked', url: url.href }, location.origin);
          }
        }
      }
    }
